(function($) {
  ...
  //localStorage.clear();
  if (localStorage.getItem("svurl") !== null) {
      $.AMUI.utils.cookie.set('svurl', localStorage.getItem("svurl"));
  } else {
      $.AMUI.utils.cookie.set('svurl', "http://i.icoolpy.com");
  }
  ...
  function appvm() {
      ...
      self.bt_setsv = function () {
          $('#my-setsv').modal({
              relatedTarget: this,
              onConfirm: function (e) {
                  localStorage.setItem('svurl', 'http://' + self.url());
                  $.AMUI.utils.cookie.set('svurl', localStorage.getItem("svurl"));
              },
              onCancel: function (e) { }
          });
      ...
      self.rembme.subscribe(function (val) {
          if (typeof (Storage) !== "undefined") {
              if (val) {
                  ...
                  localStorage.setItem('pwd', undefined);
                  self.uid("");
                  self.pwd("");
                  $("#msg").html("账号已清除！");
                  $('#my-prompt').modal('open');
      ...
      self.bt_login = function () {
          if (self.uid() !== "" && self.pwd() !== "") {
              $.AMUI.progress.start();
              $.ajax({
                  url:   $.AMUI.utils.cookie.get('svurl') + "/v1.0/user?username=" + self.uid() + "&pass=" + self.pwd()
              }).done(function (data) {
                  if (data.ukey !== null) {
